 ...
 *
 * <p>The request line uses native SLF4J key-value pairs, which Spring Boot's
 * structured encoder promotes to first-class JSON fields in production.</p>
 */
class SanitizingRequestLoggingFilterTest {
    ...
    class RequestLine {
        ...
        @Test
        @DisplayName("Should emit method, URI, status, latency and client as structured fields")
        void shouldEmitStructuredRequestLine() throws Exception {
            ...
            ILoggingEvent event = appender.list.stream()
                    .filter(e -> e.getFormattedMessage().equals("HTTP request completed"))
                    .findFirst().orElseThrow();
            assertThat(event.getKeyValuePairs())
                    .extracting(pair -> pair.key)
                    .containsExactly("method", "uri", "status", "latencyMs", "client");
            assertThat(event.getKeyValuePairs())
                    .extracting(pair -> pair.value)
                    .contains("POST", "/api/v1/projects", 200);
        }
    }
